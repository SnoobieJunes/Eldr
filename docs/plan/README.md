...
4. **Pick a workstream prefix that is not already taken.** Currently in use:
   ...
   `WS-I` (Buzz interop), `WS-BM` (Buzz mobile), `WS-B`/`WS-C`/`WS-D` (tech-week).
   Grep `DEVIATIONS.md` before minting a new one — `WS-M` was very nearly reused
   for two unrelated efforts.
