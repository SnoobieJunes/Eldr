...
| AC72 | **Metadata-file encryption (events.jsonl + eldr.md) — FOUNDATION LANDED, WIRING PENDING.** Decision (user): bring eldr-acp's redacted metadata (`events.jsonl`) and summarized context (`eldr.md`) under encryption too, so nothing conversation-derived is plaintext. **Landed + tested:** `EncryptedStore.deriveKey(label:)` (a purpose-scoped HKDF sub-key off the one SE-wrapped master key) and `PQRCACP.ACPMetadataCrypto` — a SHARED AES-GCM codec (same `nonce‖ct‖tag` layout) that BOTH the agent process and Huginn's `ContextLearner` call, so their framing can't drift; a cross-process round-trip test proves agent-seal ↔ learner-open under one derived key, plus fail-closed on wrong/corrupt key. **NOT yet done (do not claim):** wiring the codec through `ACPEventLog`/`ProjectContext.read` (agent) and `ContextLearner` (Huginn), passing the key to eldr-acp via env, and — the gating correctness issue — a SINGLE-SOURCE-OF-TRUTH, race-free master-key bootstrap (today `ConversationMemory` and the independent view-scoped `ContextLearner` would each bootstrap, and a first-run race could mint two different master keys → silent metadata-key mismatch → corrupt learning loop). Until that bootstrap is atomic (e.g. `SecItemAdd`-create-or-load) and verified on-device with the real Keychain + running agent, the files stay plaintext (already redacted/0600 per C-6). | tech-debt |
