...
- **Known pre-existing reds — RESOLVED (verified 2026-07-18 test audit):** (1) `LLMTokenAtRestTests` now passes under headless `xcodebuild test` (full Huginn suite green on this machine). (2) `perAIKeyAccount_matchesLegacyRule` was updated to the correct rule — `acp` is asserted to have NO per-AI key account (Mac-Tethered-AI's credential is the Mac-side LLM token) — and passes. Neither is a standing exception anymore; a red in either is a real regression.

**Remaining gaps / honest notes:**
- The MCP **in-app `PersonaRuntime` bridge** (A35 Phase 2) gap is **CLOSED (2026-07-18)**: `App/PQRCTests/LocalMCPGateTests.swift` covers (a) the token gate over a real loopback UDS — a bad/missing first line yields no bytes and no bridge method ever runs; (b) the redaction accessors emitting codenames ("you"/autoName, never display name or hex) + the ≤64 KB bound with no firewall input at all; (c) `stop()` severing live clients, unlinking the socket and refusing reconnects, plus `RuntimeSecureChatBridge` failing closed (reads empty, writes refused) once the model is gone. The permission-denial case ships too: `RunnerE2ETests.driverDeniesPermissionAndAgentReportsDenial` + `mutatingToolDeniedWhenClientNeverAnswers` prove a `write_file` denial is honored end-to-end. Note one latent finding: `LocalMCPServer.start()` throws `.pathTooLong` when `NSTemporaryDirectory()` exceeds sun_path's 104 bytes — fine on Mac Catalyst (short `/var/folders` paths, the actual Goose/Xcode use case), but the iOS app-container tmp path is longer, so the toggle would fail to start on-device; surface a friendlier error if that surface ever ships on iPhone.
- **Display-only chat color (A-PartyColor)** — CLOSED (2026-07-18): `App/PQRCTests/PartyColorTests.swift` asserts determinism (same hex ⇒ same resolved color) and measures WCAG contrast on the RESOLVED colors in light AND dark — white-on-solid ≥ 4.5:1 across a 24-hue sweep plus the accent-anchored self solid (deepened 22 % after the audit caught raw accent at ~4.0:1), and `.label`-on-`aiFill` for the agent wash. It never affects redaction.
