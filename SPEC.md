...
2. **Ephemeral receiving keys** (strong, OPTIONAL in v1): the recipient publishes rotating per-conversation receiving sub-keys, so the long-term identity pubkey never appears as a `p` tag. This complicates discovery and is offered as a privacy-maximizing opt-in. Because privacy is the cardinal rule (§0), implementations SHOULD work toward enabling this by default in a future version.

**Honest limitation:** against a *global passive observer* who reads all public relays, recipient + fuzzed timing leak unless ephemeral receiving keys are used. This is documented in THREAT_MODEL.md. The tradeoff buys censorship resistance and availability that a single self-run relay cannot match.
