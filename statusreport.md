...
2. Fix the **cancel/permission fail-open race** (§2.2) — order `session/cancel` before the turn resumes; keep the line-465 re-check. **(open — next)**
3. Replace the in-app ACP **auto-grant** (§2.3) with an explicit per-action or per-session mutation consent. **(open — next)**

**P1 — before exposing the ACP router to users**
4. Give the `acp` backend an honest **"not connected / replies simulated"** state (§2.5). **(open)**
5. Add **confirmation dialogs** to Block/Delete/Decline (§2.6). **(in progress this session)**
6. Apply the **G4 credential scrub** (or enforce loopback) on the PQRCACP→LLM path (§2.4). **(open)**
7. ~~Resolve the Mac node entitlements decision (§2.7)~~ **✅ done (un-sandboxed)** — still **bootstrap the owner contact** in the standalone daemon (§3) so the node is drivable. **(daemon part open)**
8. Add **replay/sequence protection** to `NearbyACPTransport` (§3). **(open)**

**P2 — quality, performance, polish**
9. Kill the **per-second full-`body` re-render** and the O(n) `threadMessages` (§3) — these will bite first on long conversations. **(open)**
10. Sweep **"PQRC" → "EldrChat"** in user-facing strings (§3) and fix the iPad/Mac reading-width + Contacts-nav issues. **(open)**
11. Crypto hygiene: **secret zeroization** and the SPEC §8.3 `participant_type` correction (§3). **(open)**
12. Decide **MCP's** future relative to ACP (§5). **(open)**
