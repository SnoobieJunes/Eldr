...
>
> **Progress (2026-06-21, session 6).** Closed the last UX/CI items: **"Me's AI" → "My AI"** (gated on the real `isMine` signal); the agent-skills **⟡⟡ envelope is stripped from chat bubbles** (display-only, raw text preserved in the record) behind a default-OFF per-silo "Show agent protocol envelope" toggle in Settings ▸ Privacy; **AI-stopped-replying** root-caused (loop guard + foreground-only AI are by-design; the silent-drop bug — swallowed provider errors in `runThreadTurn`/`runWindowReply` — is fixed); **Mac window resizable** (Catalyst `sizeRestrictions`). **CI: added an `app-build` job** that builds the EldrChat target for iOS + Mac Catalyst (fast, no signing) and gates the test job — closes the "`swift test` only builds packages-for-macOS" gap that hid an iOS-only break twice (AC36, AC41). **Accepted LOWs (reviewed, no churn):** the phone's permission allowlist trusting the node-supplied tool `kind` is advisory-by-design under C-3 (the node's own C-1 gate is authoritative); the stale `App/PQRC.xcodeproj` references in `EXTERNAL_CROSSCHECK_REVIEW.md` (a dated 06-12 review) and the stale `-scheme PQRC` entries in `.claude/settings.local.json` are harmless and left as-is.
