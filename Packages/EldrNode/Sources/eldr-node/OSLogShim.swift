...
// Non-Apple (Linux node) shim for the SUBSET of OSLog this executable uses, so the
// production call sites compile UNCHANGED and keep invariant-12 semantics: a value marked
// `privacy: .public` is logged; everything else (the privacy-first default, `.private`) is
// redacted to `<private>`. Lines go to stderr so systemd/journalctl captures them on a
// headless Linux node. Vendored per module (mirrors PQRCACP/PQRCNostr); minimal by design —
// never widen the default away from `.private` (SPEC §0).
