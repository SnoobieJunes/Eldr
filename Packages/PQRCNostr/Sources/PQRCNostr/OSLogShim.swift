...
//
// Vendored per package (mirrors PQRCACP's copy) so each module's OSLog-using files
// resolve `Logger` module-locally on Linux with no cross-module coupling. Minimal by
// design — never widen the default away from `.private` (SPEC §0).
